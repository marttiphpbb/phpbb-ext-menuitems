	...
	function main($id, $mode)
	{
		...
		switch($mode)
		{
			...
			case 'page_rendering':
				...
				$render_settings = $phpbb_container->get('marttiphpbb.menulinks.render.render_settings');

				$this->tpl_name = 'page_rendering';
				$this->page_title = $language->lang('ACP_MENULINKS_PAGE_RENDERING');
				...
				if ($request->is_set_post('submit'))
				{
					...
					$config->set('menulinks_first_weekday', $request->variable('menulinks_first_weekday', 0));
					$config->set('menulinks_min_rows', $request->variable('menulinks_min_rows', 5));
				...
				foreach ($weekdays as $value => $name)
				{
					$template->assign_block_vars('weekdays', [
						'VALUE'			=> $value,
						'S_SELECTED'	=> $config['menulinks_first_weekday'] == $value ? true : false,
				...
				$template->assign_var('MENULINKS_MIN_ROWS', $config['menulinks_min_rows']);
			...
			case 'input':
				...
				$this->tpl_name = 'input';
				$this->page_title = $language->lang('ACP_MENULINKS_INPUT');

				$input_settings = $phpbb_container->get('marttiphpbb.menulinks.render.input_settings');
			...
			case 'input_forums':
				...
				$this->page_title = $language->lang('ACP_MENULINKS_INPUT_FORUMS');
			...
			case 'include_assets':
				...
				$include_assets = $phpbb_container->get('marttiphpbb.menulinks.render.include_assets');
	
				$this->tpl_name = 'include_assets';
				$this->page_title = $language->lang('ACP_MENULINKS_INCLUDE_ASSETS');
				...
				if ($request->is_set_post('submit'))
				{
					...
					$config->set('menulinks_datepicker_theme', $request->variable('menulinks_datepicker_theme', ''));
